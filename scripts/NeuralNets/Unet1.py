...
from keras.layers import Dense, Conv2DTranspose, Activation, Conv2D, Input, AveragePooling2D, MaxPooling2D, Flatten, LeakyReLU, Dropout, Reshape
...
X_train = train.transpose().expand_dims({'var': "Prcp"}, axis=0).transpose().m
...
d1 = Conv2D(1, (1, 1), activation='linear') (c9)

d1 = Flatten()(d1)

#adding in some dense layers
d2 =Dense(3200, activation=act_fn, kernel_initializer='he_normal') (d1)
d2 = Dropout(0.1) (d2)
outputs = Dense(3200, activation=act_fn, kernel_initializer='he_normal') (d2)
outputs = Dropout(0.1) (outputs)

#reshape to grid
outputs = Reshape((40,80,1))(outputs)
...
results = model.fit(X_train, Y_train, validation_split=0.1, batch_size=batch_size, epochs=50, callbacks=[earlystopper, checkpointer])
...
train['preds'] = train.preds**4
train['preds'] = train.preds.where(train.Prcp > -1000, np.nan)
train.sel(var=0).to_netcdf('/glade/scratch/kwheelan/datasets/predictions/unet_train_predsDENSE.nc')
